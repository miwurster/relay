...
- The clone is the rig's, at a fixed path whose basename is the repo's, so the sandbox image tag is stable and its build caches between runs. The seed resets it: fetch, `reset --hard` to genesis, prune stale **pass branches** and the worktrees a crashed pass left.
- The rehearsal's **landing** is `merge`, so the **lander** runs and all eight roles are exercised.
...
The seed's refusal on a mismatched `origin` is the one part of the rig with a unit test, in `tests/` like any other: it is the guard on an unrecoverable action, and it must not be first exercised by being pointed at the wrong repo.

That the rig itself works is confirmed by running it — `npm run rehearse -- happy-path` — which is what it is for.
