...
The mount list is finalised by the topology decision: mount **`kipu-code-review` + `kipu-spec-review` + `kipu-commit`(+`caveman-commit`) + `tdd`**. The `implement`/`kipu-implement` skill is **not** mounted — the implementer runs a **custom prompt** (the leaner `implement` skill's wording minus its `/code-review` line, plus `Commit your work to the current branch using /kipu-commit`); code review is externalized to the review roles. Ticket 11 proved slash commands **do** expand headless, so `/tdd` and `/kipu-commit` are invoked by slash — this ticket's "never slash" recommendation is superseded.
