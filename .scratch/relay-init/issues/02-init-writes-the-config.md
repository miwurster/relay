...
**Init** joins `doctor` as a reserved positional on the flagless CLI. It reads `defaultBranch` from the clone rather than assuming `main`, writes the file, prints what it did, and exits 0.

The gate is not init's business: [ADR-0009](../../../docs/adr/0009-the-repos-docs-declare-the-green-gate.md) moved it out of the config and into the repo's docs, so the struck criteria below were built and then deleted. Init's report tells the operator to declare it in `AGENTS.md` instead.
...
- [x] In a GitHub clone it writes `relay.config.ts` at the repo root carrying `defaultBranch` and nothing else — defaults are not echoed back out.
- [x] The written config loads: running init in a temporary repo and then loading the result yields the branch.
...
- [x] Init prints what remains manual — declare the gate in `AGENTS.md`, create the labels, provision `GH_TOKEN` — and names `relay doctor` as the next command.
