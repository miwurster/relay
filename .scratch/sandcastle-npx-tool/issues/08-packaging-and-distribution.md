...
How does the tool become a distributable `@quantum-hub/sandcastle` runnable via `npx` in any target repo — built from scratch on the author's templates?
...
- The `bin` entry and CLI contract: `npx @quantum-hub/sandcastle [WORK-ITEM]`, no-param auto-pick, Task→`error`, exit codes.
